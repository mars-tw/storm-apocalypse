...
export const MAIN_QUESTS: readonly MainQuestDefinition[] = [
  ...
  { chapter: 6, title: "生意必須活下去", log: "只靠一雙手，撐不過下一場長夜。", target: 200, rewardGold: 50, rewardUnlock: "employee-shop", rewardLabel: "✦ 50 · 員工招募", progress: (s) => s.stats.totalEarned },
...
export interface LoopQuestDefinition {
  ...
  title: string;
  description: string;
  available: (state: RuntimeState) => boolean;
  target: (wave: number) => number;
}

export const LOOP_QUESTS: readonly LoopQuestDefinition[] = [
  { id: "stock-safe", title: "完整貨架", description: "本波不損失攤位存貨", available: () => true, target: () => 1 },
  { id: "tower-kills", title: "箭雨校準", description: "由防禦塔擊殺殭屍", available: (s) => towerCount(s) > 0, target: (wave) => 3 + Math.floor(wave / 3) },
  { id: "player-kills", title: "親手清場", description: "由玩家親手擊殺殭屍", available: () => true, target: (wave) => 2 + Math.floor(wave / 5) },
  { id: "healthy-wall", title: "不退防線", description: "以 75% 以上壁壘完成本波", available: () => true, target: () => 1 },
  { id: "frost-hits", title: "寒霜驗證", description: "以寒霜塔命中殭屍", available: (s) => s.towers.frost > 0, target: (wave) => 5 + wave },
  { id: "cannon-combo", title: "爆破紀錄", description: "單次砲擊命中至少 3 隻", available: (s) => s.towers.cannon > 0, target: () => 1 },
] as const;
